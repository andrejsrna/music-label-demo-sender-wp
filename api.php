...
function mlds_register_api_endpoints() {
	...
	register_rest_route('mlds/v1', '/test', [
		'methods' => 'GET',
	...
	register_rest_route('mlds/v1', '/feedback', [
	...
	register_rest_route('mlds/v1', '/track-info', [
		'methods' => 'GET',
	...
	register_rest_route('mlds/v1', '/subscribe', [
		'methods' => 'POST',
	...
	register_rest_route('mlds/v1', '/unsubscribe', [
		'methods' => 'POST',
	...
	register_rest_route('mlds/v1', '/update-reaction', [
		'methods' => WP_REST_Server::CREATABLE, // POST
	...
	register_rest_route('mlds/v1', '/reactions', [
		'methods' => 'GET',
...
function mlds_api_test($request) {
	return new WP_REST_Response(
		[
			'message' => __('API is working!', 'music-label-demo-sender'),
...
function mlds_api_unsubscribe($request) {
	...
	// Delete subscriber
	$result = $wpdb->delete($table_name, ['email' => $email], ['%s']);
...
function mlds_api_get_track_info($request) {
	...
	if ($token) {
		$stored_token = get_post_meta($track_id, '_mlds_track_token', true);
		if ($token !== $stored_token) {
			return new WP_Error('invalid_token', __('Invalid token.', 'music-label-demo-sender'), [
				'status' => 403,
			]);
		}
	}
	...
	if (!$attachment) {
		return new WP_Error('track_not_found', __('Track not found.', 'music-label-demo-sender'), [
			'status' => 404,
		]);
	}
...
function mlds_api_update_reaction($request) {
	...
	// Verify post exists
	if (!get_post($post_id)) {
		return new WP_Error('post_not_found', __('Post not found.', 'music-label-demo-sender'), [
			'status' => 404,
		]);
	}
